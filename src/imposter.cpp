...
Imposter::Imposter(float x, float y, float z)
{
    ...
    GLfloat vertex_buffer_data[450 + 5 * 2 * 9 + 450 + 450 + 450 + 225]; //4 rectangles
    for (int i = 0; i < 450; i++)
    {
    ...
    {
        int k = i;
        colorRectangle1[k] = colorRed[k % 3];
        if (i >= 9 && i < 12)
        {
            colorRectangle1[k] = colorDarkRed[k % 3];
        }
    }
...
void Imposter::draw(glm::mat4 VP, int flag)
{
...
void Imposter::set_position(float x, float y, float z)
{
